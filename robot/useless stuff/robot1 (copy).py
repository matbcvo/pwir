...
def omni_move(motor1, motor2, motor3):
    thrower_speed = 3000
    thrower_angle = 2700
    try:
        ser = serial.Serial(
...
def toBall(ball_y, ball_x):
    ...
    #motor2 =  #tagumine
    #motor3 =  #vasak
    omni_move(int(rightWheelLinearVelocity), int(rearWheelLinearVelocity), int(leftWheelLinearVelocity))
...
while cap.isOpened():
    ...
    img_gray = cv2.cvtColor(filtered_image, cv2.COLOR_BGR2GRAY)
